...
#this program establishes a connection to the database. It does so for the purpose of managing expenses.
...
#Several functions appear next. they manage different operations within the expense tracker application.
...
def edit_expense():
    ...
    def edit_existing_expense():
        ...
        connector.execute('UPDATE ExpenseTracker SET Date = ?, Description = ?, Amount = ?, ModeOfPayment = ? WHERE ID = ?', 
...
def selected_expense_to_words():
    ...
    message = f'Your expense can be read like. \n" You paid {values[4]} to {values[2]} for {values[1]} via {values[5]}"'
    ...
    def expense_to_words_before_adding():
        ...
        message= f'Your expense can be read like.\n "You paid {amnt.get()} to {payee.get()} for{desc.get()} om {date.get_date()} via {MoP.get()}"'
...
#This application specifies colors for backgrounds. It also sets fonts for various elements in the user interface.
...
#Initialization of GUI window occurs in this section. Basic properties receive assiognment here.
...
Label(root, text= 'EXPENSE TRACKERE', font= ('Noto Sans CJK TC', 15, 'bold'), bg = hlb_btn_bg).pack(side = TOP, fill = X)

#Certain variables hold string values. Others hold double values. they support the interface components.
...
#Frames receive creation. Positioning follows within the primary window.

data_entry_frame = Frame(root, bg = dataentery_frame_bg)
data_entry_frame.place(x=0, y=0, relheight=0.95, relwidth=0.25)
...
#Placement of elements happens inside the data entry frame. user input receives facilitation there.

Label(data_entry_frame, text = 'Date (M/DD/YY) :' ,font=lbl_font,bg=dataentery_frame_bg).place(x=10,y=230)
...
dd1=OptionMenu(data_entry_frame,MoP,*['Cash','Cheque','Credit card','Debit card ','Paytm','Google pay','Razor pay'])
...
# Buttons reside in the buttons frame. They offer quick access to various functions.
...
# The treeview frame holds the display of expenses. Presentation occurs in tabular form.
...
# Finalization of the GUI window takes place here. The main loop commences afterward.
