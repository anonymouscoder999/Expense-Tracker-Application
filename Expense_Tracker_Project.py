...
# The program connects to the database for managing expenses.
...
# The following functions handle various operations in the expense tracker.
...
# The application defines backgrounds and fonts for the user interface elements.
...
# The GUI window gets initialized here with basic properties.
root = Tk()
...
# Variables for holding string and double values in the interface.
desc = StringVar()
amnt = DoubleVar()
payee = StringVar()
MoP = StringVar(value='Cash')

# Frames are created and positioned within the main window.
...
# Elements in the data entry frame are placed for user input.
...
# Buttons in the buttons frame provide quick access to functions.
...
# The treeview frame displays the expenses in a tabular format.
...
# The GUI window is finalized and starts the main loop.
root.update()
root.mainloop()